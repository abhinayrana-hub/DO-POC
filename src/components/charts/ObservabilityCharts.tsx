...
export function ObservabilityCharts({ activeTab, page, totalHealth }: ObservabilityChartsProps) {
  if (page.id === 'pipeline') {
    return (
  ...
  if (page.id === 'api') {
    return (
      <section className="wireframe-grid api-grid">
        ...
        {page.visuals.latency && <TrendLineChart config={page.visuals.latency} />}
...
function TrendLineChart({ config }: { config: TrendChartConfig }) {
  ...
  const pointFor = (value: number, index: number, total: number) => {
    const x = padding.left + (total <= 1 ? 0 : (index / (total - 1)) * plotWidth)
    ...
    <Card className="chart-card">
      ...
      <svg className="line-chart-svg" viewBox={`0 0 ${width} ${height}`} role="img" aria-label={config.title}>
        {[0, 0.25, 0.5, 0.75, 1].map((step) => {
          ...
          return (
            <g key={step}>
              ...
              <text x={padding.left - 10} y={y + 4} className="chart-axis-text" textAnchor="end">
        ...
        {config.labels.map((label, index) => {
          if (index % Math.ceil(config.labels.length / 5) !== 0 && index !== config.labels.length - 1) {
            return null
          }
          const x = padding.left + (index / (config.labels.length - 1)) * plotWidth
          return (
            <text key={label} x={x} y={height - 12} className="chart-axis-text" textAnchor="middle">
              {label}
            </text>
          )
        })}
          ...
          return (
            <g key={series.name}>
              ...
              {points.map(([x, y], index) => (
                <circle key={`${series.name}-${index}`} cx={x} cy={y} r="3" className={`line-dot tone-${series.tone}`} />
              ))}
            </g>
          )
        })}
...
function StackedColumnChart({ config }: { config: StackedBarConfig }) {
  const max = Math.max(...config.categories.map((category) => sum(category.segments.map((segment) => segment.value))))
  ...
  return (
    <Card className="chart-card">
        ...
        {config.categories.map((category) => {
          ...
          return (
            <div className="stacked-column" key={category.label}>
              <div className="stacked-column-stack" style={{ height: `${Math.max((total / max) * 100, 8)}%` }}>
      ...
      <Legend items={config.categories[0]?.segments.map((segment) => ({ label: segment.label, tone: segment.tone })) ?? []} />
    </Card>
  )
}
  ...
  return (
    <Card className="chart-card">
      ...
      <div className="matrix-scroll">
        <div className="quality-matrix" style={{ gridTemplateColumns: `150px repeat(${config.columns.length}, minmax(98px, 1fr))` }}>
          <div className="matrix-head">Dimension</div>
          {config.columns.map((column) => (
            <div className="matrix-head" key={column}>{column}</div>
          ))}
          {config.rows.map((row) => (
            <div className="matrix-row-label" key={row.label}>{row.label}</div>
          )).flatMap((label, index) => [
            label,
            ...config.rows[index].cells.map((cell, cellIndex) => (
              <div className={`matrix-cell tone-${cell.tone}`} key={`${config.rows[index].label}-${cellIndex}`}>
                {cell.value}
              </div>
            )),
          ])}
...
function GroupedBars({ config, compact = false }: { config: GroupedBarsConfig; compact?: boolean }) {
  const max = Math.max(...config.categories.flatMap((category) => category.bars.map((bar) => bar.value)))
  ...
  return (
    <Card className={`chart-card ${compact ? 'compact-chart-card' : ''}`}>
      ...
      <Legend
        items={dedupeLegend(config.categories.flatMap((category) => category.bars.map((bar) => ({ label: bar.label, tone: bar.tone }))))}
      />
    </Card>
  )
}
...
function LineageMap({ config }: { config: LineageConfig }) {
  ...
  const nodeWidth = 170
  const nodeHeight = 28
  const positions = new Map(config.nodes.map((node) => [node.id, {
    x: 34 + node.level * 270,
    y: 28 + node.row * 48,
  }]))
  ...
  return (
    <Card className="chart-card lineage-card">
      ...
      <div className="lineage-content">
        <svg className="lineage-svg" viewBox={`0 0 ${width} ${height}`} role="img" aria-label={config.title}>
          ...
          {config.nodes.map((node) => {
            ...
            return (
              <g key={node.id}>
                <rect x={position.x} y={position.y} width={nodeWidth} height={nodeHeight} rx="3" className={`lineage-node tone-${node.tone}`} />
                <text x={position.x + 10} y={position.y + 18} className="lineage-node-text">
                  {node.label}
                </text>
              </g>
            )
...
function SignalBars({ activeTab, page, totalHealth }: ObservabilityChartsProps) {
  return (
    <Card className="chart-card">
        ...
        {page.timeline.map((item) => (
          <div className="signal-row" key={item.label}>
            ...
            <div className="signal-track">
              <div className={`signal-fill ${item.status}`} style={{ width: `${item.value}%` }} />
            </div>
...
function Legend({ items }: { items: Array<{ label: string; tone: ChartTone }> }) {
  return (
